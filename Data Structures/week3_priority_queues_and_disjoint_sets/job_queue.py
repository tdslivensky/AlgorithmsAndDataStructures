...
def MinHeapify(heap, num, start):
    ...
    if rightIndex <= start and heap[rightIndex][1] <= heap[smallestIndex][1]:
        # this if handel instances where the root and a leaf are the same the problems want to use the lowest core not just an availible core
...
# everything above copied from descheapsort
def assign_jobs(n_workers, jobs):
    result = []
    next_free_time = []
    # create a array with alll cores at time 0
    ...
    for job in jobs:
        ...
        # the next availble core is the root.
        result.append(AssignedJob(tup[0], tup[1]))
        # update the roots finish time
        end = next_free_time[0][1] + job
        next_free_time[0][1] = end
        # resort the tree from the root to get the new min
